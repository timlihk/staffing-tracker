#!/usr/bin/env python3
"""
FAST Billing Database Updater with Kimi Parser
...
class KimiMilestoneParser:
    ...
    def parse(self, text: str, struck_ordinals: Set[str] = None) -> List[Milestone]:
        ...
        for pattern, _ in self.ORDINAL_PATTERNS:
            for match in re.finditer(pattern, text, re.IGNORECASE):
                ...
                start = match.end()
                next_match = None
                for m2 in re.finditer(pattern, text[start:], re.IGNORECASE):
                    next_match = m2
                    break
                ...
                amount = self.extract_amount(segment)
...
class DatabaseUpdater:
    def __init__(self, db_url: str):
        self.db_url = db_url
        self.conn = None
        self.cm_to_project: Dict[str, int] = {}
        self.name_to_project: Dict[str, int] = {}
        self.stats = {
            'projects_updated': 0,
            'cm_no_updated': 0,
    ...
    def _load_cache(self):
        """Pre-load all project mappings into cache"""
        print("  Loading project cache...")
        with self.conn.cursor() as cur:
            # CM to project mapping
            cur.execute("""
                SELECT c.cm_no, p.project_id, p.project_name, p.client_name
                FROM billing_project p
                JOIN billing_project_cm_no c ON p.project_id = c.project_id
            """)
            for row in cur.fetchall():
                cm_no, project_id, project_name, client_name = row
                self.cm_to_project[cm_no] = project_id
                # Also index by partial CM number
                if '-' in cm_no:
                    base_cm = cm_no.split('-')[0]
                    self.cm_to_project[base_cm] = project_id
            
            # Name to project mapping
            cur.execute("SELECT project_id, project_name, client_name FROM billing_project")
            for row in cur.fetchall():
                project_id, project_name, client_name = row
                if project_name:
                    self.name_to_project[project_name.lower()] = project_id
                if client_name:
                    self.name_to_project[client_name.lower()] = project_id
        
        print(f"    Cached {len(self.cm_to_project)} CM numbers, {len(self.name_to_project)} names")
    ...
    def get_project_by_cm(self, cm_no: str) -> Optional[int]:
        ...
        # Direct lookup
        if cm_no in self.cm_to_project:
            return self.cm_to_project[cm_no]
        
        # Partial match
        for cached_cm, project_id in self.cm_to_project.items():
    ...
    def get_project_by_name(self, project_name: str, client_name: str) -> Optional[int]:
        if project_name:
            ...
            if key in self.name_to_project:
                return self.name_to_project[key]
        ...
        with self.conn.cursor() as cur:
            cur.execute("""
                UPDATE billing_project SET
                    ...
                    updated_at = NOW()
                WHERE project_id = %s
            """, (data.client_name, data.attorney_in_charge, data.sca, project_id))
        ...
        with self.conn.cursor() as cur:
            ...
            if result:
                ...
                cur.execute("""
                    ...
                    WHERE cm_id = %s
                """, (
                    data.billing_usd, data.collection_usd, data.ubt_usd,
                    data.billing_credit_usd, data.billing_credit_cny, data.ubt_cny,
                    cm_id
                ))
            else:
                cur.execute("""
                    INSERT INTO billing_project_cm_no (
                        ...
                        ubt_usd, billing_credit_usd, billing_credit_cny, ubt_cny,
                        financials_updated_at
                    ) VALUES (%s, %s, FALSE, 'active', %s, %s, %s, %s, %s, %s, NOW())
                    RETURNING cm_id
                """, (
                    project_id, cm_no, data.billing_usd, data.collection_usd,
                    data.ubt_usd, data.billing_credit_usd, data.billing_credit_cny, data.ubt_cny
                ))
        ...
        with self.conn.cursor() as cur:
            ...
            if result:
                ...
                cur.execute("""
                    ...
                    SET raw_text = %s, parsed_at = NOW(), parser_version = 'kimi-v1', updated_at = NOW()
                    WHERE fee_id = %s
    ...
    def upsert_milestones(self, engagement_id: int, fee_id: int, milestones: List[Milestone]):
        with self.conn.cursor() as cur:
            for i, m in enumerate(milestones):
                ...
                if result:
                    milestone_id, existing_completed = result
                    cur.execute("""
                        UPDATE billing_milestone SET
                            description = %s, amount_value = %s, amount_currency = %s,
                            percent_value = %s, is_percent = %s, completed = %s,
                            ...
                            updated_at = NOW(), sort_order = %s, raw_fragment = %s
                        WHERE milestone_id = %s
                    """, (
                        m.description, m.amount, m.currency, m.percent, m.percent is not None,
                        m.completed, m.completed, m.completed, i, m.description[:100], milestone_id
                    ))
                    self.stats['milestones_updated'] += 1
                else:
                    cur.execute("""
                        INSERT INTO billing_milestone (
                            engagement_id, fee_id, ordinal, description, amount_value, amount_currency,
                            percent_value, is_percent, completed, completion_source, completion_date,
                            created_at, updated_at, sort_order, raw_fragment
                        ) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, NOW(), NOW(), %s, %s)
                    """, (
                        engagement_id, fee_id, m.ordinal, m.description, m.amount, m.currency,
                        m.percent, m.percent is not None, m.completed,
...
def load_excel_data(excel_path: str, specific_rows: List[int] = None) -> List[ProjectData]:
    ...
    print("  Extracting strikethrough data...")
    struck_cells = {}  # row_idx -> set of ordinals
    ...
    for min_row, max_row in rows_to_process:
        for idx, row in enumerate(sheet_data.iter_rows(min_row=min_row, max_row=max_row, values_only=True), start=min_row):
            ...
            cell_strike = False
            try:
                cell = sheet_format.cell(idx, 9)
                if cell.font:
                    cell_strike = cell.font.strike or False
            except Exception:
                pass
            
            # Build struck ordinals set
            struck_ordinals = set()
            if cell_strike:
                struck_ordinals.add("*")
            
            # Also check XML-level strikethrough
            for ss in ss_list:
                if ss['has_strike'] and fee_text and len(fee_text) > 20:
                    fee_start = fee_text[:50].replace('\n', ' ')
                    ss_start = ss['full_text'][:50].replace('\n', ' ')
                    if fee_start in ss_start or ss_start in fee_start:
...
def main():
    ...
    print("="*80)
    print("🤖 KIMI BILLING DATABASE UPDATER (FAST)")
    print("="*80)
    ...
    updater = DatabaseUpdater(args.db_url)
    ...
    try:
        ...
        for i, project in enumerate(projects, 1):
            ...
            if not project_id:
                not_found += 1
                if not_found <= 5:  # Only show first 5 not found
                    print(f"  ⚠️ Row {project.row_num}: Not found - CM={project.cm_no}")
                continue
            ...
            # Progress every 5 projects + commit
            if processed % 5 == 0:
                updater.commit()
                print(f"  Progress: {i}/{total} projects ({matched} matched, {not_found} not found)")
