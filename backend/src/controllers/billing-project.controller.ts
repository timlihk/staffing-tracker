...
export async function getBillingProjectDetail(req: AuthRequest, res: Response) {
  try {
    ...
    // Run access check + project info in parallel (both needed before branching)
    const [hasAccess, projectData, bcAttorneys] = await Promise.all([
      canAccessBillingProject(projectIdBigInt, req.user),
      prisma.$queryRaw<BillingProjectRow[]>`
        ${BILLING_DASHBOARD_SELECT}
        WHERE project_id = ${projectIdBigInt}
        LIMIT 1
      `,
      prisma.$queryRaw<BCAttorneyRow[]>`
        SELECT
    ...
    if (view === 'summary') {
      const [summaryData, eventCount] = await Promise.all([
        prisma.$queryRaw<CMSummaryRow[]>`
          SELECT
            cm.cm_id,
            cm.cm_no,
            cm.is_primary,
    ...
    const fetchEvents = cmIdFilter === null && engagementIdFilter === null;
    const [fullData, events, financeComments] = await Promise.all([
      ...
      fetchEvents
        ? prisma.$queryRaw<BillingEventRow[]>`
            ...
            LIMIT 50
          `
        : Promise.resolve([] as BillingEventRow[]),
      fetchEvents
        ? prisma.$queryRaw<FinanceCommentRow[]>`
            SELECT *
            ...
            LIMIT 100
          `
        : Promise.resolve([] as FinanceCommentRow[]),
    ]);
